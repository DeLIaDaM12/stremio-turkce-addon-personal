...
class DiziWatchProvider(BaseProvider):
    ...
    @staticmethod
    def _series_episode_url(base_url: str, meta: MediaMeta) -> str:
        if meta.media_type != "series" or not meta.season or not meta.episode:
            return base_url
        return f"{base_url.rstrip('/')}-{meta.season}-sezon-{meta.episode}-bolum-izle/"
        ...
        async with httpx.AsyncClient(headers=headers, timeout=8.0, follow_redirects=True, verify=False) as client:
            ...
            for query in meta.search_queries:
                try:
                    ...
                    for item in soup.select(".post-content a, .entry-title a, article a, .title a"):
                        href = item.get("href")
                        if not href:
                            continue
                        ...
                        target_url = self._series_episode_url(full_url, meta)
                        break
            ...
            try:
                ...
                # catch direct HLS URLs in any tag
                ...
                for tag in page_soup.select("iframe, source, script, a[href*='.m3u8'], a[href*='playlist']"):
                    raw = tag.get("src") or tag.get("data-src") or tag.get("href")
                    if not raw:
                        continue
